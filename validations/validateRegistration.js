...
function checkPassword(res, password) {
  if (password === undefined) {
    sendStatusError(400, MESSAGE_ERROR3, res);
  } else if (typeof (password) !== 'string') {
    sendStatusError(400, MESSAGE_ERROR1, res);
  } else if (password.length <= 5) sendStatusError(400, MESSAGE_ERROR2, res);
}

function checkEmail(res, email, emailRegex) {
  if (email === undefined) {
    sendStatusError(400, MESSAGE_ERROR4, res);
  } else if (!emailRegex.test(email)) sendStatusError(400, MESSAGE_ERROR1, res);
}

function checkDisplayName(res, displayName) {
  if (displayName.length <= 7) {
    sendStatusError(400, MESSAGE_ERROR6, res);
  } else if (displayName === undefined) {
    sendStatusError(400, MESSAGE_ERROR5, res);
